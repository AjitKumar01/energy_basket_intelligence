...
from audit_particle_counterfactual_generation import resolve_initialization_artifact
...
def test_additive_resurrection_validates_initialization_lineage(tmp_path):
...
def test_full_additive_resurrection_rejects_unfinished_latest_checkpoint(tmp_path):
    initialization = tmp_path / "initialization.pt"
...
def test_rank_and_candidate_resurrection_validation(tmp_path):
    initialization = tmp_path / "initialization.pt"
...
def test_certification_rejects_failed_or_corrupt_evaluation_bundle(tmp_path, monkeypatch):
    ...
    pipeline.validate_evaluation_outputs(profile="full", dry_run=False)
