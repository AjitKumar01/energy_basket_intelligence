...
def pair_energy(x: sparse.csr_matrix, phi: np.ndarray) -> np.ndarray:
    totals = x @ phi
    return 0.5 * (np.square(totals).sum(1) - x @ np.square(phi).sum(1))


def pair_adjoint(x: sparse.csr_matrix, phi: np.ndarray,
                 weight: np.ndarray) -> np.ndarray:
    """Gradient of sum_b weight_b sum_{i<j in b} <phi_i,phi_j>."""
    return x.T @ (weight[:, None] * (x @ phi)) - phi * (x.T @ weight)[:, None]
